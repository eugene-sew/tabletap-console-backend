...
class Tenant(TenantMixin):
    ...
    def save(self, *args, **kwargs):
        ...
        if not is_new:
            try:
                old_slug = Tenant.objects.get(pk=self.pk).slug
        ...
        # Update associated domain if slug changed (and not new)
        if not is_new and old_slug and self.slug != old_slug:
            from .models import Domain
            primary_domain = Domain.objects.filter(tenant=self, is_primary=True).first()
            if primary_domain:
                new_domain = f"{self.slug}.tabletap.space"
