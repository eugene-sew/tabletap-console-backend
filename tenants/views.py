...
def restaurant_current(request):
    ...
    try:
        ...
        # Transparent Discovery: If we are in the 'public' schema but a user is logged in,
        # try to find their actual tenant and switch the connection so we return their data.
        from django_tenants.utils import get_public_schema_name
        if tenant.schema_name == get_public_schema_name() and request.user.is_authenticated:
            # Try to resolve by clerk_organization_id (which is currently the clerk_user_id for owners)
            tenant_match = Tenant.objects.filter(clerk_organization_id=request.user.clerk_user_id).first()
            if tenant_match:
                from django.db import connection
                tenant = tenant_match
                connection.set_tenant(tenant)
                # Note: We don't update request.tenant globally here to avoid side effects,
                # but we use 'tenant' for the rest of this view.
        ...
        if request.method == 'GET':
            ...
            # Ensure they can only update if they are an owner of THIS tenant
            # This is a bit simplified, but essential for security if we use discovery
            if tenant.schema_name == get_public_schema_name():
                return Response({'error': 'Cannot update public restaurant info'}, status=400)
